...
{
    unsigned char result[SHA256_DIGEST_LENGTH * 2 + 1] = {0};
    SHA256_CTX ctx;
    SHA256_Init(&ctx);
    SHA256_Update(&ctx, in.data(), in.size());
    SHA256_Final(&result[SHA256_DIGEST_LENGTH], &ctx);
    ...
    for (int32_t i = 0; i < SHA256_DIGEST_LENGTH; ++i) {
        unsigned char value = result[SHA256_DIGEST_LENGTH + i];
        // uint8_t is 2 digits in hexadecimal.
        result[i * DOUBLE_TIMES] = hexCode[(value >> WIDTH) & MASK];
        result[i * DOUBLE_TIMES + 1] = hexCode[value & MASK];
    }
    result[SHA256_DIGEST_LENGTH * DOUBLE_TIMES] = 0;

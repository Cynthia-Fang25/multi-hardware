...
bool AVSyncManager::MergeGroupInfo(std::string &syncGroupInfo)
{
    ...
    for (const auto &item : streamInfoList_) {
        if ((item.sceneType == SCENE_TYPE_D_MIC) || (item.sceneType == SCENE_TYPE_D_SPEAKER)) {
            ...
            cJSON_AddNumberToObject(masterStr, KEY_AV_SYNC_FLAG.c_str(), AvSyncFlag::MASTER);
            ...
            cJSON_AddNumberToObject(slaveStr, KEY_AV_SYNC_FLAG.c_str(), AvSyncFlag::SLAVE);
    ...
    char *jsonstr = cJSON_Print(jsonStr);
    if (jsonstr == nullptr) {

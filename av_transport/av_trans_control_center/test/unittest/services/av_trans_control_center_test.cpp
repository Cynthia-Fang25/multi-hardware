...
{
    int32_t engineId = 1;
    sptr<IAVTransControlCenterCallback> callback = new CenterCallback();
    int32_t ret = center_->RegisterCtlCenterCallback(engineId, callback);
    EXPECT_EQ(DH_AVT_SUCCESS, ret);
}
...
{
    AVTransEvent event;
    //event.content = (uint32_t)AVTransTag::START_AV_SYNC;
...
HWTEST_F(AVTransControlCenterTest, set_param_2_engines_001, TestSize.Level0)
{
    ...
    int32_t ret = center_->SetParam2Engines(tag, value);

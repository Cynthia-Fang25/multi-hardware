...
void AVTransControlCenterCallbackTest::SetUp()
{
    ...
    callBack_->receiverEngine_ = std::shared_ptr<AVReceiverEngine>(ownerName, peerDevId);
    callBack_->senderEngine_ = std::shared_ptr<AVSenderEngine>(ownerName, peerDevId);

void AVTransControlCenterCallbackTest::TearDown()
{
    callBack_->senderEngine_ = nullptr;
    callBack_->receiverEngine_ = nullptr;
    callBack_ = nullptr;
}
...
/**
 * @tc.name: set_sender_engine_001
 * @tc.desc: set sender engine function.
 * @tc.type: FUNC
 * @tc.require: AR000GHSK9
 */
HWTEST_F(AVTransControlCenterCallbackTest, set_sender_engine_001, TestSize.Level0)
{
    std::string ownerName = "ownerName1";
    std::string peerDevId = "peerDevId1";
    std::shared_ptr<IAVSenderEngine> sender = std::shared_ptr<AVSenderEngine>(ownerName, peerDevId);
    callBack_->SetSenderEngine(sender);
    EXPECT_EQ(callBack_->senderEngine_, sender);
}

/**
 * @tc.name: set_receiver_engine_001
 * @tc.desc: set receiver engine function.
 * @tc.type: FUNC
 * @tc.require: AR000GHSK9
 */
HWTEST_F(AVTransControlCenterCallbackTest, set_receiver_engine_001, TestSize.Level0)
{
    std::string ownerName = "ownerName1";
    std::string peerDevId = "peerDevId1";
    std::shared_ptr<IAVReceiverEngine> receiver = std::shared_ptr<AVReceiverEngine>(ownerName, peerDevId);
    callBack_->SetReceiverEngine(receiver);
    EXPECT_EQ(callBack_->receiverEngine_, receiver);
}
    ...
    DHType dhType, EnableParam &param, std::shared_ptr<CapabilityInfo> &capability)
{
    ...
    DHLOGI("GetEnableCapParam success. dhType = %{public}#X, sink uuid =%{public}s, sink dhId = %{public}s,"
        "sinVersion = %{public}s, source uuid =%{public}s, source dhId = %{public}s, sourceVersion = %{public}s,"
        "subtype = %{public}s", dhType, GetAnonyString(uuid).c_str(), GetAnonyString(dhId).c_str(),

...
std::shared_ptr MetaInfoManager::GetInstance()
{
    static std::shared_ptr instance(new(std::nothrow) MetaInfoManager);
    if (instance == nullptr) {
...
MetaInfoManager::MetaInfoManagerEventHandler::MetaInfoManagerEventHandler(
    const std::shared_ptrAppExecFwk::EventRunner &runner, std::shared_ptr metaInfoMgrPtr)
    : AppExecFwk::EventHandler(runner)
{
    DHLOGI("Ctor MetaInfoManagerEventHandler");
    MetaInfoMgrWPtr_ = metaInfoMgrPtr;
}
...
std::shared_ptrMetaInfoManager::MetaInfoManagerEventHandler MetaInfoManager::GetEventHandler()
{
...
int32_t MetaInfoManager::Init()
{
    ...
    std::lock_guardstd::mutex lock(metaInfoMgrMutex_);
    dbAdapterPtr_ = std::make_shared(META_APP_ID, GLOBAL_METAINFO_ID, shared_from_this());
    ...
    std::shared_ptrAppExecFwk::EventRunner runner = AppExecFwk::EventRunner::Create(true);
    eventHandler_ = std::make_sharedMetaInfoManager::MetaInfoManagerEventHandler(runner, shared_from_this());
...
int32_t MetaInfoManager::SyncMetaInfoFromDB(const std::string &deviceId)
{
    ...
    std::vectorstd::string dataVector;
    ...
    for (const auto &data : dataVector) {
        ...
        if (CapabilityUtils::GetCapabilityByValue(data, metaCapInfo) != DH_FWK_SUCCESS) {
